...
import { fetchDiscountByPointAction } from "@/actions/discountApiAction";
import { useState } from "react";
import { Input } from "../Input";

type DiscountPointProps = {
  cartId: string;
  discount: number;
  setDiscount: (discount: number) => void;
};

export default function DiscountPointBlock(
  props: DiscountPointProps
) {
  const [point, setPoint] = useState(0);
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const response = await fetchDiscountByPointAction(props.cartId, point);
    if (response.data) {
