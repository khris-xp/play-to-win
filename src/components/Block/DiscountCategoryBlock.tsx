...
import { fetchDiscountByCategoryAction } from "@/actions/discountApiAction";
...
export default function DiscountCategoryBlock(
  props: DiscountCategoryProps
) {
  const [category, setCategory] = useState("");
  const [percentage, setPercentage] = useState(0);
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const response = await fetchDiscountByCategoryAction(
      props.cartId,
      category,
      percentage
    );
    ...
    <div className="space-y-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:p-6">
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div>
          ...
          <Input
            type="text"
            placeholder="Enter your discount category"
            className="w-full"
            value={category}
